...
void Navigation::Run() {
  ...
  if ((odom_loc_ - waypoints.front()).norm() < 0.1) {
    waypoints.pop();
  }
  ...
  const Vector2f current_waypoint = waypoints.front();
  cout << current_waypoint << endl;
  ...
  if ((odom_loc_ - human_loc_).norm() > 1.0 || ((odom_loc_ - door_loc).norm() < 2.0 && door_state != OPEN)) {
    state_ = STOPPED;
  } else {
